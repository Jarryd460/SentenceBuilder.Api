...
internal sealed class ApiHealthCheck : IHealthCheck
{
    ...
    public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
    {
        ...
        var response = await httpClient.GetAsync($"{baseUrl}api/information/version").ConfigureAwait(false);

        if (response.IsSuccessStatusCode)
        {
            return await Task.FromResult(new HealthCheckResult(
              status: HealthStatus.Healthy,
              description: "The Api is up and running.")).ConfigureAwait(false);
        }

        return await Task.FromResult(new HealthCheckResult(
          status: HealthStatus.Unhealthy,
          description: "The Api is down.")).ConfigureAwait(false);
    }
}
